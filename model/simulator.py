...
from .utils import calc_spiral_angle, calc_sigma2_glong, calc_sigma2_glat, calc_sigma2_vlsr

def simulator(theta, Rmin=torch.tensor(3.0), Rmax=torch.tensor(15.0),
              Rref=torch.tensor(8.0), fixed={}, disk=None):
    """
    ...
    spiral_az = torch.stack(tuple(torch.linspace(mina, maxa, 1000) for mina, maxa in zip(min_az, max_az)))
    if disk is not None:
        # apply exponential disk
        I2, Rs, Rc = disk
        spiral_R = Rref*torch.exp((params['az0'][:, None] - spiral_az)*torch.tan(params['pitch'][:, None]))
        prob = torch.exp(-spiral_R/Rs)/(1.0 + I2*torch.exp(-spiral_R/Rc))
        prob = prob/torch.sum(prob, axis=1, keepdims=True)
        az = torch.tensor([choice(saz, 1, True, p)[0] for saz, p in zip(spiral_az, prob)])
    else:
    ...
    glong, glat, vlsr, dvlsr_ddist, dist = model(
        az, params['az0'], torch.tan(params['pitch']),
        params['R0'], params['Usun'], params['Vsun'], params['Wsun'],
        params['Upec'], params['Vpec'],
        cos_tilt, sin_tilt, cos_roll, sin_roll, R0a22, lam, loglam,
        term1, term2, theta0, params['warp_amp'], params['warp_off'],
        Rref=Rref)
    #
